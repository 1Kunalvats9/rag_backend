...
export const embedFileChunks = async (req, res) => {
    try {
        ...
        // Get chunks
        const chunks = await prisma.chunk.findMany({ where: { fileId: fileId } });
        if (chunks.length === 0)
        ...
        for (const chunk of chunks) {
            ...
            await prisma.$executeRawUnsafe(`UPDATE "Chunk" SET embedding = $1::vector WHERE id = $2`, vectorString, chunk.id);
        }
